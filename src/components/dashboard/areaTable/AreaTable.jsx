...
import "./AreaTable.scss";
 ...
 const calculatePercentageDifference = (actual, planned) => {
  return ((actual - planned) / planned) * 100;
};

const AreaTable = () => {
  const filteredOpenMarketData = data.filter(item => item['Market Type'] === 'Open Market').slice(0, 10);
  const filteredFacilitiesData = data.filter(item => item['Market Type'] === 'Facilities').slice(0, 10);
  const combinedData = [...filteredOpenMarketData, ...filteredFacilitiesData];

  // Sort the combined data by GWP in descending order
  combinedData.sort((a, b) => b.GWP - a.GWP);

  // Take the top 10 from the combined data
  const top10CombinedData = combinedData.slice(0, 10);

  return (
    <div className="area-table-container">
      <div className="table-container">
        <h2>Top 10 Open Market Brokers</h2>
        <table>
          <thead>
            <tr>
              <th>Rank</th>
          ...
          <tbody>
            {filteredOpenMarketData.map((broker, index) => (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>{broker.Broker_Name}</td>
                <td>{broker.GWP}</td>
                <td>{broker['Planned GWP']}</td>
                <td>{calculatePercentageDifference(broker.GWP, broker['Planned GWP']).toFixed(2)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Repeat the above structure for Facilities table */}

      <div className="table-container">
        <h2>Combined Top 10 Brokers</h2>
        <table>
          <thead>
            <tr>
              <th>Rank</th>
          ...
          <tbody>
            {top10CombinedData.map((broker, index) => (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>{broker.Broker_Name}</td>
                <td>{broker.GWP}</td>
